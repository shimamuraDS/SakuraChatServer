...
同一个 ChatServer 程序使用不同配置运行两个节点，连接共享 MySQL。新增事务保存文本消息、UUID 幂等重试、历史分页和送达/已读回执；客户端与节点需要一起升级到协议版本 2。

部署前提、协议和实现边界见 [聊天持久化与回执说明](docs/CHAT_HISTORY_READ_RECEIPTS.md)。缺少聊天表时使用增量脚本 [002_chat_history.sql](database/002_chat_history.sql)，已有不同结构的表需单独核对；不要用包含 DROP TABLE 的完整建库脚本升级。已完成编译检查，未运行应用或测试，未执行 SQL。

## 历史好友功能进度（2026-09-08，仅记录当时状态）
